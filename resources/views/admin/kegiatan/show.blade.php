                ...
                <div class="row">
                    <div class="col-md-12 mb-3">
                        ...
                        <input type="text" class="form-control @error('nama') is-invalid @enderror"
                            ...
                            value="{{ old('nama', $kegiatan->nama) }}" readonly>
                        @error('nama')
                        ...
                        <select name="tipe" id="tipe"
                            ...
                            @foreach ($option_kegiatan as $item)
                                <option value="{{ $item->parameter }}"
                                    {{ old('tipe', $kegiatan->tipe) == $item->parameter ? 'selected' : '' }}>
                                    {{ $item->isi }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col-md-12 mb-3">
                        ...
                        <textarea class="form-control @error('keterangan') is-invalid @enderror" placeholder="Masukkan keterangan..."
                            id="keterangan" name="keterangan" readonly>{{ old('keterangan', $kegiatan->keterangan) }}</textarea>
                        @error('keterangan')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
